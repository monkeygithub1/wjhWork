...
public class Loan_006 {
	...
	//一次性通过
	@Test
	public void loan_006() throws ClientProtocolException, IOException {
		Map<String, String> headermap = restClient.header();//获取信息头（带cookie）
		String loanId = "";
		request.login(headermap, phone_create);//登录
		request.loan_bsfApply(headermap, money_, type_, proj_name, proj_id_, loanId);
	...
	//每个节点拒绝一次
	@Test(priority=1)
	public void loan_006_Refuse() throws ClientProtocolException, IOException {
		Map<String, String> headermap = restClient.header();//获取信息头（带cookie）
		String loanId = "";
		request.login(headermap, phone_create);//登录
		request.loan_bsfApply(headermap, money_, type_, proj_name, proj_id_, loanId);
	...
	public void loanEdit (Map<String, String> headermap, String loanId) throws ClientProtocolException, IOException{
		...
		request.loan_bsfUpdate(headermap, money_, type_, proj_name, proj_id_, loanId);
	}
}
