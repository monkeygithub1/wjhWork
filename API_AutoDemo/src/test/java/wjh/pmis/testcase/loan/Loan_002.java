...
import wjh.pmis.restclient.RestClient;
	/*
	 * 职能部门（安质部）-办公费-综合管理部审批
	 * 最后一个节点为事业部财务
	 */
public class Loan_002 {
	...
	//一次性通过
	@Test
	public void loan_002() throws ClientProtocolException, IOException {
	...
	//每个节点拒绝一次
	@Test(priority=1)
	public void loan_002_Refuse() throws ClientProtocolException, IOException {
		Map<String, String> headermap = restClient.header();//获取信息头（带cookie）
		String loanId = "";
		request.login(headermap, phone_create);//登录
		Map<String, String> para = new RequestPara().loan_para(money_, type_, proj_name, proj_id_, loanId);
