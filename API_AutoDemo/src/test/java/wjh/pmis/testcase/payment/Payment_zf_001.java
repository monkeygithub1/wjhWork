...
import wjh.pmis.testcase.purchase.Purchase_zf_001;
	/*
	 * 职能部门（安质部）
	 * 采购类型：7-租房采购
	 * 是否电商：1-电商
	 */
public class Payment_zf_001 {
	...
	public void setUp() throws InterruptedException, ClientProtocolException, IOException {
		purchaseId = new Purchase_zf_001().purchase_zf_001();
	}
	//一次性通过
	@Test
	public void payment_zf_001() throws ClientProtocolException, IOException {
		...
		request.paymentSp(headermap, phone_zhglzz1, paymentId, "1", "顺顺ZZ");//综合管理
	...
	//每个节点拒绝一次
	@Test(priority=1)
	public void payment_zf_001_Refuse() throws ClientProtocolException, IOException {
		Map<String, String> headermap = restClient.header();//获取信息头（带cookie）
		String paymentId = "";
		String proc_id_ = "";
		request.login(headermap, phone_create);//登录
		...
		request.paymentSp(headermap, phone_fg, paymentId, "1", "顺顺ZZ");
		request.paymentSp(headermap, phone_cwmgr, paymentId, "1", "顺顺ZZ");
		request.paymentSp(headermap, phone_zhglzz1, paymentId, "2", "顺顺ZZ");
		//综合管理拒绝
		this.paymentEdit(headermap, paymentId, proc_id_);
		request.paymentSp(headermap, phone_pm, paymentId, "1", "顺顺ZZ");
		request.paymentSp(headermap, phone_dept, paymentId, "1", "顺顺ZZ");
		request.paymentSp(headermap, phone_fg, paymentId, "1", "顺顺ZZ");
		request.paymentSp(headermap, phone_cwmgr, paymentId, "1", "顺顺ZZ");
		request.paymentSp(headermap, phone_zhglzz1, paymentId, "1", "顺顺ZZ");
