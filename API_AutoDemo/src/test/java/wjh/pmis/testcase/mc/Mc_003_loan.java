...
public class Mc_003_loan {
	...
	public void mc_002_Refuse() throws ClientProtocolException, IOException {
		...
		request.mcClaim(headermap, phone_mc_claim_bzj, mcId, type_mc, ywId, update_time_, deptId);//认领
		//认领有误，重新认领
		request.mcApprove(headermap, phone_mc_approve_bzj, mcId, "2", "有误");//确认
		this.mcEdit(headermap, mcId);
		update_time_ = request.getUpdateTimeId(headermap);
		request.mcClaim(headermap, phone_mc_claim_bzj, mcId, type_mc, ywId, update_time_, deptId);
		request.mcApprove(headermap, phone_mc_approve_bzj, mcId, "1", "");
